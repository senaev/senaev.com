...
rsync -avz --delete -e ssh "$SCRIPT_DIR/provisioning/" "$DEPLOY_HOST:$K3S_CLUSTER_PATH/provisioning/"
echo "✅ Provisioning files synced"

echo "👉 Deploying k8s cluster to server"
ssh "$DEPLOY_HOST" "$K3S_CLUSTER_PATH/provisioning/k8s/scripts/deploy-k8s.sh"
